...
@interface CMBaseTableView : UITableView<UITableViewDelegate,UITableViewDataSource>
/*
 是否为group类型的（分组table）
 默认是NO,如果为YES,则
...
/*
  点击Cell时的回调
 */
@property(nonatomic,copy) void(^selectHanlder)(id model);

/*
   刷新Cell时回调，如果仅仅根据model刷新cell可以不必设定
    重写cell的-(void)yc_setModelData:(id)data;方法即可刷新数据
    如果有额外操作时可设定该值
 */
@property(nonatomic,copy) void(^refreshCell)(UITableViewCell* cell,id model);
